...
class ModelCurso():
    ...
    def get_by_user(self, db, user_id):
        try:
            cursor = db.connection.cursor()
            sql = """SELECT id, user_id, nombre, informacion, time_create, time_update FROM cursos WHERE user_id = {}""".format(user_id)
            cursor.execute(sql)
            row = cursor.fetchall()
            if row != None:
                new_row = list()
                for curso in row:
                    ...
                    sentimientos = ModelReview.get_sentimientos(db, curso[0])
                    if sentimientos != None:
                        curso_list += sentimientos
                    new_row.append(curso_list)
                return new_row
            else:
                return None
        except Exception as ex:
